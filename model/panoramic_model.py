...
from model import base_function, test_net
import itertools
...
class panoramic(BaseModel):
    def name(self):
        return "Panoramic Feature Aggregation Network for Large Mask Inpainting"
    ...
    def modify_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_l1', type=float, default=10, help='weight for reconstruction loss l1')
            ...
            parser.add_argument('--lambda_highper', type=float, default=5, help='weight for high receptive pl loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['app_g', 'ad_g', 'img_d', 'highper', 'sty']
        self.visual_names = ['img_m', 'img_truth', 'img_out', 'img_g']
        self.model_names = ['G', 'D',]

        # define the inpainting model
        self.net_G = test_net.define_g(gpu_ids=opt.gpu_ids)
        # define the discriminator model
        self.net_D = test_net.define_d(gpu_ids=opt.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.sty = StyleLoss()
            self.highper = HRFPL(weight=5, weights_path=os.getcwd())
            # define the optimizer
            #itertools.chain():可以将多个可迭代对象合并成一个迭代器，即将多个序列按顺序连接起来
    ...
    def backward_G(self):
        ...
        self.loss_highper = self.highper(self.img_truth * (1 - self.mask), self.img_g * (1 - self.mask))  # * self.opt.lambda_highper

        totalG_loss = self.loss_app_g + self.loss_sty + self.loss_ad_g + self.loss_highper # + self.loss_per
